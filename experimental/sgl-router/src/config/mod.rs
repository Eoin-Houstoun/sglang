...
/// Ceiling on `shutdown_drain_secs`, enforced by [`Config::validate`]. Rejected
/// rather than advised, because no grace period makes a pause this long
/// serviceable: past here the value is a typo (`500` for `50`), and accepting
/// it would turn every later termination into a SIGKILL through the middle of
/// the streaming responses the drain exists to protect.
pub const MAX_SHUTDOWN_DRAIN_SECS: u64 = 300;
        ...
        if self.server.shutdown_drain_secs > MAX_SHUTDOWN_DRAIN_SECS {
            return Err(anyhow!(
                "shutdown_drain_secs must be at most {MAX_SHUTDOWN_DRAIN_SECS} (got {}); \
                 a pause that long cannot be serviced under any plausible \
                 terminationGracePeriodSeconds, so every termination would be a SIGKILL",
