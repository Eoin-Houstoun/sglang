...
mod tests {
    ...
    /// `Config`: a fat-fingered `500` for `50` must fail at startup rather
    /// than turn every later termination into a SIGKILL.
    #[test]
    fn shutdown_drain_secs_past_the_ceiling_is_rejected() {
        ...
        .expect_err("a 500s drain must not start")
        .to_string();
        assert!(
